const firstNames = ["Ana", "Bruno", "Carla", "Daniel", "Eduarda", "Felipe", "Gabriela", "Henrique", "Isabela", "Joao", "Karina", "Lucas", "Mariana", "Nicolas", "Olivia", "Pedro", "Rafaela", "Samuel", "Talita", "Vinicius", "Yasmin", "Caio", "Bianca", "Davi", "Helena"];
    ...
    function loadData() {
      try {
        ...
        if (Array.isArray(saved) && saved.length) {
          students = saved;
          rooms = sortRooms([...new Set(saved.map(item => item.className))]);
          return;
        }
    ...
    function getFilteredGroups() {
      ...
      return rooms
        ...
        .map(room => {
          const roomStudents = students.filter(item => {
            if (item.className !== room) return false;
            const haystack = normalize(`${item.className} ${item.studentName} ${item.schoolDoc} ${item.parentsName} ${item.contactName} ${item.contactPhone}`);
                ...
                ${group.students.map(item => `
                  <tr>
                    ...
                    <td>
                      <div class="actions">
                        ${whatsappLink(item)
                          ? `<a class="btn whatsapp small-btn" target="_blank" rel="noopener" href="${whatsappLink(item)}">WhatsApp</a>`
                          : `<button class="btn whatsapp small-btn" type="button" data-action="invalid-phone" data-id="${item.id}">WhatsApp</button>`}
                    ...
                    <td><span class="phone">${escapeHtml(item.contactPhone)}</span></td>
                  </tr>
                `).join("")}
              </tbody>
            </table>
    ...
    function exportCsv() {
      ...
      const header = ["Sala", "Nome do aluno", "Documento escolar", "Nome dos pais", "Nome do contato telefonico", "Numero do contato telefonico"];
    ...
    function buildAttendanceSheetXml(room, month, year) {
      ...
      const lastColumn = columnName(2 + days.length);
      ...
      const headerCells = [
        sheetCell("A3", "Serie", 2),
        sheetCell("B3", "Nome do aluno", 2)
      ];
      days.forEach((day, index) => {
        headerCells.push(sheetCell(`${columnName(index + 3)}3`, String(day).padStart(2, "0"), 2));
      });
      ...
      roomStudents.forEach((item, index) => {
        ...
        const cells = [
          sheetCell(`A${rowNumber}`, item.className, 4),
          sheetCell(`B${rowNumber}`, item.studentName, 4)
        ];
        days.forEach((day, dayIndex) => {
          cells.push(sheetCell(`${columnName(dayIndex + 3)}${rowNumber}`, "", 3));
        });
        rowsXml.push(sheetRow(rowNumber, cells));
      });

      const validationRange = days.length && roomStudents.length
        ? `<dataValidations count="1"><dataValidation type="list" allowBlank="1" showErrorMessage="1" sqref="C4:${lastColumn}${lastRow}"><formula1>"P,A,F"</formula1></dataValidation></dataValidations>`
        : "";

      return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships">
  <dimension ref="A1:${lastColumn}${lastRow}"/>
  <sheetViews><sheetView workbookViewId="0"><pane xSplit="2" ySplit="3" topLeftCell="C4" activePane="bottomRight" state="frozen"/></sheetView></sheetViews>
  <cols><col min="1" max="1" width="12" customWidth="1"/><col min="2" max="2" width="34" customWidth="1"/><col min="3" max="${2 + days.length}" width="4" customWidth="1"/></cols>
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles>
    ...
    roomsList.addEventListener("click", event => {
      ...
      if (action === "invalid-phone") {
        const item = students.find(student => String(student.id) === String(id));
        alert(`Cadastre um numero valido com DDD para abrir o WhatsApp direto${item ? ` de ${item.studentName}` : ""}. Sem numero, o WhatsApp obriga escolher um contato.`);
      }
    });
